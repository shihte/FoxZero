...
# 1. 清除舊權重 (防止繼承中毒的模型)
rm -f models/foxzero_model.pth models/foxzero_weights_sync.pth
rm -f logs/gpu*.log
...
nohup env CUDA_VISIBLE_DEVICES=0 PYTHONPATH=. python3 foxzero/train_parallel.py \
  ...
  --sims 200 \
  --batch_size 64 \
  --max_updates 5000 > logs/gpu0_baseline.log 2>&1 &
PID0=$!
echo "實驗 A (GPU 0) 啟動 PID: $PID0 | Log: logs/gpu0_baseline.log"
...
nohup env CUDA_VISIBLE_DEVICES=1 PYTHONPATH=. python3 foxzero/train_parallel.py \
  ...
  --sims 50 \
  --batch_size 128 \
  --max_updates 10000 > logs/gpu1_fast.log 2>&1 &
PID1=$!
echo "實驗 B (GPU 1) 啟動 PID: $PID1 | Log: logs/gpu1_fast.log"
...
nohup env CUDA_VISIBLE_DEVICES=2 PYTHONPATH=. python3 foxzero/train_parallel.py \
  ...
  --sims 100 \
  --batch_size 256 \
  --max_updates 5000 > logs/gpu2_explore.log 2>&1 &
PID2=$!
echo "實驗 C (GPU 2) 啟動 PID: $PID2 | Log: logs/gpu2_explore.log"

echo "---------------------------------------------------"
echo "監控指令 (即時查看 Buffer 成長):"
echo "tail -f logs/gpu*.log | grep 'Buffer'"